...
class GoodsLogic
{
    ...
    public static function lists($get)
    {
        ...
        if(isset($get['pro_city']) && $get['pro_city']){
            $where[] = ['pro_city','like','%'.$get['pro_city'].'%'];
        }
		 if(isset($get['city']) && $get['city']){
            $where[] = ['s.city','like','%'.$get['city'].'%'];
        }
		//mydev
        $goods_count = Db::name('goods')->alias("a")
		        ->join("supplier s","a.supplier_id=s.id")
                ->where($where)
                ->count();
        ...
        $goods_list = Db::name('goods')->alias("a")
                ...
                ->field('a.*,virtual_sales_sum+sales_sum as total_sales_sum,s.city')
                ->order('id desc')
                ->select();
    ...
    public static function add($post, $spec_lists)
    {
        ...
        try {
            ...
            $searchStr = 'src="'. $domain . '/uploads';
            $replaceStr = 'src="/uploads';
            $post['content'] = str_replace($searchStr, $replaceStr, $post['content']);
...
//                'shareholder_ratio'         => $post['shareholder_ratio'],
                ...
                'create_time'               => $time,
                'goods_arg'                =>json_encode($post['goods_arg'])
            ];
...
//                'shareholder_ratio'         => $post['shareholder_ratio'],
    ...
    public static function info($goods_id)
    {
        ...
        $info['base'] = Db::name('goods')
		 ...
		 if($info['base']['goods_arg']){
		 	 $info['base']['goods_arg']=json_decode($info['base']['goods_arg'],true);
		 }else{
		 	 $info['base']['goods_arg']=array();
		 }
