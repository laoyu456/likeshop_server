...
class UserLogic{
    ...
    public static function lists($get){
        ...
        foreach ($user_list as &$item){
            ...
            if ($item['avatar'] != '/static/common/image/default/user.png') {
                $item['abs_avatar'] = $item['avatar'] ? UrlServer::getFileUrl($item['avatar'],'local') : '';
            } else {
            ...
            if(isset($all_user_list[$item['first_leader']])){
                 ...
                 $item['referrer_avatar'] = UrlServer::getFileUrl($referrer_user['avatar'], 'local');
    ...
    public static function getUser($id,$getdata = false,$expenditure = false){
        ...
        $user['abs_avatar'] = UrlServer::getFileUrl($user['avatar'], 'local');
        if($expenditure){
            //本月的消费金额
