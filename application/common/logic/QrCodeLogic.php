...
class QrCodeLogic extends LogicBase{
    ...
    public function makeGoodsPoster($user,$goods,$url,$url_type){
        ...
        try {
            ...
            $goods_image = UrlServer::getFileUrl($goods['image'],'share')?:ConfigServer::get('website', 'goods_image');
//            $goods_image = $goods['image']?:ConfigServer::get('website', 'goods_image');
    ...
    public function makeUserPoster($user, $content, $url_type, $client)
    {
        try {
            ...
            $user_avatar = file_exists('.'.$user['avatar']) ? UrlServer::getFileUrl($user['avatar'],'share') : ROOT_PATH.ConfigServer::get('website', 'user_image');
            $qr_code_logic = new QrCodeLogic();
