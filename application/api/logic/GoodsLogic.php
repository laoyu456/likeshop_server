...
class GoodsLogic{
    ...
    public static function getGoodsList($user_id,$get,$page,$size){
        ...
        if(isset($get['category_id']) && $get['category_id']){
        	//mydev
        	if(strpos($get['category_id'],"_")!==FALSE){
        		  $narr=explode("_",$get['category_id']);
				  $where[]=['is_new','=',$narr[1]];
				  $is_sur=1;
        	}else{
        ...
        if($is_sur){
        	 ...
        	 $where[]=['stype','=',$narr[0]];
			 if($narr[0]==1){
			 	 $city=$get["city"]?$get["city"]:"南京";
				 $city=str_replace("市","",$city);
				 $where[] = ['s.city','like','%'.$city.'%'];
				 //print_r($where);die;
			 }
        	 $goods_count = $goods->alias("g")
             ...
             $goods_list = $goods
                    ...
                    ->page($page,$size)
                    ->order($order)
                    ->field('g.id,g.name,image,min_price as price,market_price,sales_sum+virtual_sales_sum as sales_sum')
                    ->select();
        }else{
    ...
    public static function getGoodsDetail($user_id,$id){
        ...
        if($goods){
           ...
           if($user_id) {
            ...
            //商品规格
            $goods->GoodsSpec();
            $goods->append(['comment'])->hidden(['Spec','GoodsSpecValue'])->visible(['id','name','image','video','stock','remark','goods_arg','goods_arg_info','content','sales_sum','click_count','price','market_price','is_collect','goods_spec','goods_image','goods_item','activity']);
